    ...
    */
    ...
    void station_distance (int i, int j) {
        ...
        while (not q.empty()) {
            ...
            for (int k = 0; k < 8; ++k) {
                ...
                if (pos_ok (r) and not visit[r.i][r.j]) {
                    visit[r.i][r.j] = true;
                    if (cell(r).type == Road or cell(r).type == Desert) {
    ...
    void move_warriors() {
        ...
        for (int id : w) {
            ...
            //Si estic a la ciutat i hi ha un cotxe esperant -> no sortir
            if (cell(unit(id).pos).type == City and car_here(unit(id).pos)) {
                command(id, Dir(None));
                next_pos.insert(unit(id).pos + Dir(None));
            }
            //Si nececita aigua, ves a reomplir
            else if (unit(id).water <= water[unit(id).pos.i][unit(id).pos.j] + 1) {
            ...
            //Mirem quants warriors tenim per ciutat i si son 2 o menys, defensem
            else if (cell(unit(id).pos).type == City and cell(unit(id).pos).owner == me() and defenders[city_my] <= 2 and defenders[city_my] >= 0) {
    ...
    void move_cars() {
        ...
        for (int id : c) {
            if (can_move(id)) {
                ...
                if (unit(id).food <= station[unit(id).pos.i][unit(id).pos.j] + 10) {
                    int min_dist = infinite;
                    int go = 8;
                    for (int k = 0; k < 8; ++k) {
                        Pos p = unit(id).pos + Dir(k);
                        if (pos_ok(p) and min_dist > station[p.i][p.j] and next_pos.find(p) == next_pos.end()) {
                            if (cell(p).id == -1 or (unit(cell(p).id).player != me() and unit(cell(p).id).type == Warrior)) {
                                go = k;
                ...
                //Busca enemic mes proper i ataca'l
                    ...
                    for (int k = 0; k < 8; ++k) {
                        Pos p = unit(id).pos + Dir(k);
                        if (pos_ok(p) and min_dist > enemy[p.i][p.j] and next_pos.find(p) == next_pos.end()) {
                            if (cell(p).id == -1 or (unit(cell(p).id).player != me() and unit(cell(p).id).type == Warrior)) {
                                go = k;
                                min_dist = enemy[p.i][p.j];
                            }

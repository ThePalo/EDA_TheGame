    ...
    */
        ...
        while (not q.empty()) {
            ...
            for (int k = 0; k < 8; ++k) {
                ...
                if (pos_ok (r) and not visit[r.i][r.j]) {
                    visit[r.i][r.j] = true;
                    if (cell(r).type == City) {
                        //Acaba si la unitat de la casella es d'un altre jugador
                        if (cell(r).id != -1 and unit(cell(r).id).player == me()) ++defenders;
                        q.push(r);
                    }
                }
            }
    ...
    void move_warriors() {
        ...
        for (int id : w) {
            ...
            else if (cell(unit(id).pos).type == City and cell(unit(id).pos).owner == me() and count_defenders(unit(id).pos.i, unit(id).pos.j) <= 2) {
